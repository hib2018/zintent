...
func TestWorkspaceUsesFramedIntentMainAndStatusPanes(t *testing.T) {
	m := NewWorkspace()
	m.Width, m.Height = 100, 28
	output := m.View().Content
	for _, title := range []string{"Workspace", "[ Intents ]", "Main: intents", "Status"} {
		if !strings.Contains(output, title) {
			t.Fatalf("missing pane %q:\n%s", title, output)
		}
	}
	if strings.Contains(output, "┌ Nav ") {
		t.Fatalf("redundant navigation pane is still rendered:\n%s", output)
	}
